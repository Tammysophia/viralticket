...
const AdminGPTAgents = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingAgent && editingAgent !== 'new') {
        await updateAgent(editingAgent, formData);
        toast.success('✅ Agente atualizado!');
      } else {
        const payload = { ...formData, order: agents.length + 1 };
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Deseja excluir o agente "${name}"?`)) return;
    try {
  ...
  const handleToggleActive = async (agent) => {
    try {
      ...
      console.error('VT: Toggle active error:', error);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">
              Configure os links dos agentes ChatGPT disponíveis para os usuários
            </p>
          </div>
          <Button
            onClick={() => {
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0">
                    ...
                    <div className={agent.imageUrl ? 'text-4xl hidden' : 'text-4xl'}>
                      {agent.icon || '🤖'}
                    </div>
                  </div>
